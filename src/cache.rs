...
//!
//! Cache key = sha256(language + body + param_hash). Env/secret identity,
//! included-file hashes, and runtime versions are **not** yet part of the key
//! (tracked under #33/#18); change the cache id or pass `--no-cache` when those
//! change.
...
/// Compute the cache key for a block: sha256(language\0body\0param_hash), 16 hex.
pub fn cache_key(language: &str, body: &str, param_hash: Option<&str>) -> String {
...
mod tests {
    ...
    #[test]
    fn key_changes_with_body_and_params() {
        let a = cache_key("bash", "echo 1", None);
        let b = cache_key("bash", "echo 2", None);
        let c = cache_key("bash", "echo 1", Some("ff00"));
        assert_ne!(a, b);
        assert_ne!(a, c);
        assert_eq!(a, cache_key("bash", "echo 1", None));
        assert_eq!(a.len(), 16);
    }
    ...
    fn roundtrip_serialization() {
        let mut s = CacheStore::default();
        s.record(cache_key("bash", "x", None), true, 0, "t");
