...
- [ ] `reap_expired` should acquire the per-ckpt file lock before mutating — currently uses a sibling reap lock that serializes reapers against each other but not against a live `wb run` (follow-up to #17 / surfaced during #24). Tracked as Phase 3 of the #29 work.
